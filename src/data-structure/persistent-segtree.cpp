template<typename Monoid> struct PersistentSegmentTree {
   ...
   Node* update(int a, const Monoid& x, Node* k, int l, int r) {
      ...
      else return merge(update(a, x, k->l, l, (l + r) >> 1), update(a, x, k->r, (l + r) >> 1, r));
   }

   Node* update(Node* t, int k, const Monoid& x) { return update(k, x, t, 0, sz); }

   Monoid query(int a, int b, Node* k, int l, int r) {
      if(r <= a || b <= l) return M1;
      else if(a <= l && r <= b) return k->data;
      else return f(query(a, b, k->l, l, (l + r) >> 1), query(a, b, k->r, (l + r) >> 1, r));
   }

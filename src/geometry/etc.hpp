// 1: a-bから見てa-cが反時計回り -1: a-bから見てa-cが時計回り
// 0: a-c-bがこの順で直線
// 2: c-a-bの順で直線 -2: a-b-cの順で直線
...
Point reflection(const Point& p, const Line& l) { return 2.0 * projection(p, l) - p; }     // 65ba76
bool isinterPL(const Point& p, const Line& l) { return abs(p - projection(p, l)) < EPS; }  // e9d393
bool isinterPS(const Point& p, const Line& s) { return ccw(s[0], s[1], p) == 0; }          // 79c17b
...
double distancePL(const Point& p, const Line& l) { return abs(p - projection(p, l)); }  // c77772
